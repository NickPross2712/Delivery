...
def solution(node, idx):
    if idx < 0:
        return node  # Возвращаем неизменённый список, если индекс отрицательный
    ...
    for i in range(idx - 1):
        if current is None:
            return node  # Если индекс больше длины списка, ничего не меняем
        current = current.next_item

    if current is None or current.next_item is None:
        return node  # Если текущий элемент или следующий не существует, ничего не меняем
